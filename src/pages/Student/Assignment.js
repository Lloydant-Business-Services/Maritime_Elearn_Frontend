...
class StudentAssignment extends Component {
	state = {
		...
		maxCharacter: this.props.location.state.data?.maxCharacters,
		pageLoading: false,
		thisAssignment: [],
		colorNow:"green",
	...
	submitAssignment = (e) => {
		...
		SubmissionProps.append("AssignmentId", this.state.thisAssignment.assignmentId);
		
		if (this.state.submissionText) {
			SubmissionProps.append("AssignmentInText", this.state.submissionText);
		}
		
		if (this.state.submissionLink) {
			SubmissionProps.append("AssignmentHostedLink", this.state.submissionLink);
		}
		
		if (this.state.submissionFile) {
			SubmissionProps.append("AssignmentUpload", this.state.submissionFile);
		}
		
		Endpoint.submitStudentAssignment(SubmissionProps)
			.then((res) => {
	...
	loadDataFromServer = () => {
		...
		let queryProps = {
			assignmentId: this.props.location.state.data.assignmentId,
			studentId: user.userId
		};
		Endpoint.getAssignment(this.props.location.state.data.assignmentId)
			.then((res) => {
				console.log(res.data, "asiiiii");
				this.setState({assignmentDetails: res.data}); //assignmentInText assignmentInstruction
			})
			.catch((error) => {
				this.loadDataError(error, this);
				this.setState({pageLoading: false, })
	...
	onChangeSubmissionText = (e) => {
		let __text = e.target.value;
		const chrAllowed = chrCount
		this.setState({
			maxCharacter: chrAllowed - __text.length, 
			submissionText: e.target.value
		})
		if(__text.length == chrAllowed){
			this.setState({
				colorNow:"red"
			})
		}
	...
	componentDidMount() {
		chrCount = this.state.maxCharacter
		...
		return (
			<>
				...
				<div className="container-fluid py-5">
					<div className="">
						<h1 className="mb-3 text-primary" style={{fontSize:"16px"}}>
							<Unicons.UilPen size="24" className="mr-1"/>
							{this.state.thisAssignment.assignmentName} | <span className="h2">{this.state.thisAssignment.courseTitle}</span>
						</h1>
					</div>
					...
					<div className="row">
								...
								<div className="card-body">
									<p className="mb-3">
										...
										<span className="small">
											{this.state.assignmentDetails?.assignmentInstruction}
										</span>
									</p>
									...
									<p className="mt-1 small mb-0 text-danger">
										...
										<span className="font-weight-bold">Due On: </span>
						...
						<div className="col-md-12 col-lg-6">
							<div className="card">
								<div className="card-body">
									<p className="mb-3">
										<span className="font-weight-600">Assignment: </span>
										<span className="small">
											<b>
											{this.state.assignmentDetails?.assignmentInText}
											</b>
										</span>
									</p>
							...
							<hr/>
							{
								this.state.submitted ?
									<div>
										...
										<div className="row">
											{
												this.state.submission && this.state.submission.assignmentInTextSubmission ?
													<div className="col-md-12 my-3">
														...
														<textarea cols="30" rows="3" className="form-control"
																  ...
																  value={this.state.submission.assignmentInTextSubmission}
																  readOnly>
														</textarea>
													</div>
													:
									...
									<div>
										<h3>My Submission &nbsp; &nbsp; 
											{chrCount >= 1 ? <span style={{fontSize:"11px", color:`${this.state.colorNow}`}}>character limit: {this.state.maxCharacter}</span> : null}
											</h3>
										
										<form onSubmit={(e) => this.promptSubmit(e)}>
											<div className="row">
												<div className="col-md-12 mb-3">
										<textarea maxLength={chrCount <= 0 ? 500000 : chrCount} id='submissionArea' cols="30" rows="5" className="form-control" placeholder="Answer in text" 
										// value={this.state.submissionText}
												  onChange={(e) => this.onChangeSubmissionText(e)}>
										</textarea>
												</div>
