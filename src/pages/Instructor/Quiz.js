...
class Quiz extends Component {
	...
	publishAssignment = (e) => {
		...
		let publishProps = {
			quizId: this.state.thisAssignment.quizId,
			publish: true
		};
		console.log(publishProps);
		
		Endpoint.publishQuizScores(publishProps)
			.then((res) => {
				console.log(res.data);
	...
	loadDataFromServer = () => {
		...
		Endpoint.getQuiz(this.props.location.state.data.quizId)
			.then((res) => {
				console.log(res.data);
		...
		Endpoint.getQuizSubmissions(this.props.location.state.data.quizId)
			.then((res) => {
				console.log(res.data);
				...
				<div className="container-fluid py-5">
					<div className="d-flex flex-wrap justify-content-between">
						<h1 className="mb-0 text-primary">
							...
							{this.state.thisAssignment.quizName} <small style={{fontSize:"13px"}}>(Quiz)</small> | <span className="h2 mr-2">{this.state.thisAssignment.courseTitle}</span>
							{
								this.state.thisAssignment.isPublished ?
									...
									<>
										{/* &nbsp;&nbsp; |&nbsp; <span style={{fontSize:"14px"}}>Published</span><img src={rrr} style={{width:"50px"}}/> &nbsp; */}
										&nbsp;&nbsp; |&nbsp; <span style={{fontSize:"14px"}}>Quiz Grade Status:</span><img src={pending2} style={{width:"93px"}}/> &nbsp; 
								...
								<div className="card bg-custom-light">
									<div className="card-body">
										<div className="mb-4" style={{fontSize:"14px"}}>
											...
											{this.state.thisAssignment.quizInstruction}
										</div>
							...
							<div className="col-lg-7">
								<div className="card-body">
									<div className="mb-4">
										...
										{this.state.thisAssignment.quizInText}
									</div>
									
									{
										this.state.thisAssignment.quizUploadLink ?
											<div className="mb-4">
												<a href={this.state.thisAssignment.quizUploadLink}
									...
									{
										this.state.thisAssignment.quizVideoLink ?
											<div className="mb-4">
												<a href={this.state.thisAssignment.quizVideoLink} target="_blank" className="font-weight-600">
									...
									
									{
										!this.state.thisAssignment.isPublished ?
											<button className="btn btn-primary btn-round" style={{background:"#2a7163", border:"none"}}
													onClick={this.openPromptPublish}>Publish Quiz Scores</button>
											:
											null
									}
									...
									this.state.submissions.map((submission, index) => {
										return (
											<tr key={index}>
												...
												<td>
													<Link to={{pathname:"/instructor/quizsubmission", state:{data:submission}}}>
